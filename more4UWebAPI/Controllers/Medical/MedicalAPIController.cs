...
namespace MoreForYou.APIController.Medical
{
    ...
    public class MedicalAPIController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetAllMedicalData(MedicalUserAPIModel medicalUserAPIModel)
        {
            ...
            string Country = "";
            var currentEmployee = await _employeeService.GetEmployee(medicalUserAPIModel.UserNumber);
            ...
            {
                return BadRequest(new { Flag = false, Message = "invalid User", Data = 0 });
            }
            List<MedicalCategoryModel> medicalCategoryModels = await _medicalCategoryService.GetAllMedicalCategories();
            if (medicalCategoryModels != null && medicalCategoryModels.Count > 0)
            {
                medicalCategoryAPIModels = _medicalCategoryService.ConvertMedicalCategoriesModelToMedicalCategoriesAPIModel(medicalCategoryModels, medicalUserAPIModel.LanguageId);
            }
            ...
            {
                medicalSubCategoryAPIModels = _medicalSubCategoryService.ConvertMedicalSubCategoriesModelToMedicalSubCategoriesAPIModel(medicalSubCategoryModels, medicalUserAPIModel.LanguageId);
            }
            List<MedicalDetailsModel> medicalDetailModels = await _medicalDetailsService.GetMedicalDetailsForCountry(Country);

            if (medicalDetailModels != null && medicalDetailModels.Count > 0)
            {
                medicalDetailsAPIModels = _medicalDetailsService.ConvertMedicalDetailsModelToMedicalDetailsAPIModel(medicalDetailModels, medicalUserAPIModel.LanguageId);
            }
